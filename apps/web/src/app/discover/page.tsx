...
function DiscoverWorkspace({ user }: { user: AuthUser }) {
  ...
  useEffect(() => {
    const loadMeta = async () => {
      try {
        ...
        // The signed-in profile still provides usable defaults when metadata is unavailable.
      }
    };
    void loadMeta();
  }, []);
  ...
  return (
    <AppShell user={user} title="Find a practice partner" subtitle="Search only returns complete, discoverable profiles. Each result shows the deterministic overlap that brought it here.">
