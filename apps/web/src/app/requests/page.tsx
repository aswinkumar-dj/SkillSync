...
import { RequestsSkeleton } from "../../components/ui/page-skeletons";
...
const statusClass = (status: string) => {
  ...
  return "bg-white/[0.05] text-muted";
};
...
function RequestCard({
  ...
  return (
    <article className="rounded-[22px] bg-white/[0.04] p-5 shadow-[0_24px_60px_rgba(0,0,0,0.2)] backdrop-blur-xl">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted">
            {person.targetRole ?? "Interview practice partner"}
            {person.yearsOfExperience != null ? ` · ${person.yearsOfExperience}y exp` : ""}
          </p>
        </div>
        <span className={`rounded-[12px] px-3 py-2 text-xs font-semibold ${statusClass(request.status)}`}>
          {request.status.toLowerCase()}
        </span>
      </div>

      {request.message ? (
        <p className="mt-4 rounded-[16px] bg-black/20 px-4 py-3 text-sm leading-6 text-muted">
          {request.message}
        </p>
      ) : (
        <p className="mt-4 text-sm text-muted">No note included.</p>
      )}

      <div className="mt-4 flex flex-wrap gap-2">
        {request.requestedRole ? (
          <span className="rounded-[10px] bg-white/[0.04] px-2.5 py-1 text-xs text-orange-100">
            {request.requestedRole}
          </span>
        ) : null}
        {request.requestedTopics.map((topic) => (
          <span key={topic} className="rounded-[10px] bg-white/[0.04] px-2.5 py-1 text-xs text-muted">
            {topic}
          </span>
        ))}
      </div>
      ...
      {request.match ? (
        <div className="mt-4 flex flex-wrap items-center justify-between gap-3 rounded-[16px] bg-emerald-500/10 px-4 py-3">
          <p className="text-sm text-emerald-100">Private match created.</p>
          <Link
            href={`/matches/${request.match.id}`}
            className="rounded-[12px] bg-emerald-500/20 px-3 py-2 text-xs font-semibold text-emerald-50 transition hover:bg-emerald-500/30"
          >
            Open match
          </Link>
        </div>
          ...
          {direction === "incoming" ? (
            <>
              ...
              <button
                ...
                className="rounded-[14px] bg-white/[0.05] px-4 py-3 text-sm font-medium text-muted hover:text-text disabled:opacity-60"
              >
                Decline
              </button>
            </>
...
function RequestsWorkspace({ user }: { user: AuthUser }) {
  ...
  const act = async (id: string, action: "accept" | "decline" | "cancel") => {
    ...
    try {
      ...
      }>(`/api/v1/match-requests/${id}/${action}`, {
        method: "POST",
      });

      await load();

      if (action === "accept" && response.data.match?.id) {
        router.push(`/matches/${response.data.match.id}`);
      }
    } catch (actionError) {
  ...
  return (
    <AppShell
      ...
      subtitle="Review incoming and outgoing practice invitations before a private match is created."
    >
      {error ? (
          ...
          <section>
            ...
            <div className="grid gap-4">
              {incoming.length ? (
                ...
                <EmptyState copy="No one has requested a practice match yet." />
              )}
            </div>
          </section>
          ...
          <section>
            ...
            <div className="grid gap-4">
              {outgoing.length ? (
                ...
                <EmptyState copy="Send a request from Discover when someone looks aligned." />
              )}
            </div>
          </section>
        </div>
...
export default function RequestsPage() {
  return <AuthGate mode="dashboard">{(user) => <RequestsWorkspace user={user} />}</AuthGate>;
}
